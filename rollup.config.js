        ...
        Object.entries(source).forEach(([name, value]) => {
            if (name in object) {
                ...
                if (Array.isArray(objectValue)) {
                    if (name === "plugins") {
                        const pluginNames = objectValue.map(
                            (plugin) => plugin.name
                        );
...
function devConfigs() {
    ...
    return defineConfig({
        input: { [outputFileName]: "src/main.js" },
        output: {
            dir: "dist",
            format: "iife",
            sourcemap: "inline",
        ...
        return {
            ...
            buildStart(options) {
                ...
                if (!headerPlugin) {
                    ...
                    throw new Error(
                        `This plugin depends on the "${pluginName}" plugin.`
                    );
                }
            ...
            generateBundle(options) {
                ...
                const devFileContent =
                    headerPluginApi?.generateUserScriptHeader(
                        userScriptHeaderContent
                    ) ?? "";
